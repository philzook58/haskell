...
[ (a, bimapl f g) ] -- sort of applicative feeling? function wrappred in vector context that we're applying

-- was never super clear that applicative isn't better fit for vector than monad.
-- sort of would make linear maps into linear sums of porjections or permutations.
-- which is true.

--linear arrows has to be linear function of v which is [(a,e)]
v -l> w -l> [a ,(v,w)]

-- component wise 
-- the idea is that instead of saying kron is like using basis (e,e)
-- instead it is an enclosing context that contains two vectors implemented god knows how
-- and we only operate via the lbind that the enclosed vector expose.
